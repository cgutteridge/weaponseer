...
export default function processSpreadStep(
  battleSpread: Spread<string>,
  defenders: Defender[]
): Spread<string> {
  ...
  battleSpread.forEach((spreadItem) => {
    ...
    stepResults.forEach((stepResult) => {
      addToSpread(newBattleSpread, stepResult.item, stepResult.probability * spreadItem.probability)
    })
  })
